 ...
 */
const DisplayOTPSecret = ({ open, onOpenChange, uid }: IDisplayOTPSecretProps) => {
  /* **********************************************************************************************
   *                                             STATE                                            *
   ********************************************************************************************** */
  const { data, loading, error } = useAPIRequest<string>(UserService.getOTPSecret, [uid]);
  ...
  if (error) {
    ...
    content = (
    <>
      <div className='flex justify-start items-center border border-slate-300 rounded-md p-5 shadow-md'>
        <KeyRound aria-hidden='true' className='mr-2' /> <p>{data}</p>
      ...
      <p className='text-light text-sm text-center'>The OTP Secret is a sensitive piece of data. Make sure to share it with great care</p>
    </>
    );
  }
  return (
