...
import { ChartCandlestick, Droplet, Bitcoin, Undo2, ArrowLeftRight, Server } from 'lucide-react';
...
const Adjustments = () => {
  ...
  return (
    <>
      <div className="page-container flex justify-center items-start animate-in fade-in duration-700">
        <section className="w-full md:w-8/12 lg:w-6/12 xl:w-5/12 2xl:w-4/12">
          <header className="flex justify-start items-center">
            <h1 className="text-2xl font-semibold leading-none tracking-tight">Adjustments</h1>
              ...
              <CardContent className="pt-0 md:p-0">
                {FORMS.map((form, i) => (
                  <Fragment key={form.id}>
                    ...
                    >
                      {form.icon}
                      <div className="ml-2 max-w-[85%] sm:max-w-[90%]">
       ...
       * FORM DIALOGS *
       *************** */}
      {activeDialog === 'WINDOW' && <Window closeDialog={setActiveDialog} />}
      {activeDialog === 'LIQUIDITY' && <Liquidity closeDialog={setActiveDialog} />}
      {activeDialog === 'COINS' && <Coins closeDialog={setActiveDialog} />}
      {activeDialog === 'REVERSAL' && <Reversal closeDialog={setActiveDialog} />}
      {activeDialog === 'STRATEGY' && <Strategy closeDialog={setActiveDialog} />}
      {activeDialog === 'SERVER_ALARMS' && <ServerAlarms closeDialog={setActiveDialog} />}
    </>
  );
};
