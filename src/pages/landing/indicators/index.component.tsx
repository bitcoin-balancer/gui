...
import { Bitcoin, ChartCandlestick, Droplet, Undo2 } from 'lucide-react';
 ...
 */
const Indicator = ({ title, description, openDialog }: IIndicatorProps) => (
  <Card className="border-solid shadow-sm w-10/12 sm:w-full">
    <CardContent className="text-center flex flex-col items-center p-6 md:p-6">
...
const Indicators = memo(({ openLargeInfoDialog }: IIndicatorsProps) => (
  <div className="w-full flex justify-center items-start">
    <section className="w-full sm:w-11/12 md:w-9/12 lg:w-7/12 xl:w-10/12 2xl:w-9/12 p-3">
      <header className="text-center">
        <h2 className="text-center text-4xl sm:text-5xl font-bold">Indicators</h2>
        <p className="text-xs sm:text-sm mt-2 text-light">
          Tools used by Balancer to monitor the state of the market
        </p>
      </header>

      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-8 mt-10 justify-items-center">
        <Indicator
          title="Window"
